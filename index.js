...
class ManageGame {
  ...
  static getNewGame() {
    if (ManageGame.myGame === null) ManageGame.myGame = new ManageGame();
  ...
  onNewClick() {
    ...
    document.getElementById("scorePlayer2").innerText =
      this.currentPlayer.currentScore = 0;
    document.getElementById("scorePlayer1").innerText =
      this.currentPlayer.currentScore = 0;
    document.getElementById("currentScore1").innerText =
      this.currentPlayer.currentScore = 0;
    document.getElementById("currentScore2").innerText =
      this.currentPlayer.currentScore = 0;
    ...
    document.getElementById("currentScoreBox" + this.player).style.display =
      null;
  }
  ...
  onDiceClick() {
    ...
    if (this.myDice.myCurrentFace === 1) {
      ...
      document.getElementById("playerName" + this.player).innerText =
  ...
  onHoldClick() {
    this.players[this.currentUser].totalScore =
      this.players[this.currentUser].totalScore +
      this.players[this.currentUser].currentScore;
    this.players[this.currentUser].currentScore = 0;
    ...
    if (this.players[this.currentUser].totalScore >= this.scoreMax) {
      ...
      document.getElementById("currentScoreBox" + this.player).style.display =
        "none";
      document.getElementById("playerWinner" + this.player).innerText =
        this.players[this.currentUser].name + " a gagné";
  ...
  changeUser() {
    ...
    if (this.currentUser === 0) this.currentUser = 1;
...
const changeDice = (myDice) => {
  document
    .getElementById("diceImage")
    .classList.remove(getMyClassName(myDice.oldFace));
  document
    .getElementById("diceImage")
    .classList.add(getMyClassName(myDice.myCurrentFace));
};
